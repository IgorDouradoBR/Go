...
type Barrier struct {
	...
	catraca1 *MCCSemaforo.Semaphore
	catraca2 *MCCSemaforo.Semaphore
	count    int
	n        int
}
...
func NewBarrier(val int) *Barrier {
	b := &Barrier{
		...
		catraca1: MCCSemaforo.NewSemaphore(0),
		catraca2: MCCSemaforo.NewSemaphore(1),
		count:    0,
		n:        val,
	}
...
func useBarrier(b *Barrier, i int) {
	for {
		fmt.Print(i, " ")
		b.Arrive()
		fmt.Print(i, " ")
